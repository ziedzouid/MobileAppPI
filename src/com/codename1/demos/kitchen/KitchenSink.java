...
public class KitchenSink {
    ...
    static Container tabletSurface;
    static Command gridCommand;
    static Command listCommand;

    static Object imageMask;
    static int maskWidth;
    static int maskHeight;
    static Object circleMask;
    static int circleMaskWidth;
    static int circleMaskHeight;
    ...
    private void showSplashAnimation() {
        ...
        ScaleImageLabel iconBackground = new ScaleImageLabel(res.getImage("carpool.png"));
        ...
        Label iconForeground = new Label(res.getImage("carpool.png"));
        ...
        Display.getInstance().callSerially(() -> {
            ...
            Label kitchenSink = new Label("CarCoOl", "SplashTitle");
    ...
    void showMainUI() {
        String account;
        if (UserDAO.user.getRole().equals("ROLE_CONDUCTEUR")) {
            account = "PILOT";
        } else {
            account = "PASSENGER";
        }

        final Form f = new Form(account, new BorderLayout());
        ...
        // System.out.println(UserDAO.user.prenom+".jpg");
        //String im = UserDAO.user.prenom.toLowerCase()+".jpg".toLowerCase();
        //System.out.println(im);
        System.out.println(UserDAO.user.getAvatar());
        g.drawImage(res.getImage(UserDAO.user.getAvatar()), 0, 0, circleMaskWidth, circleMaskHeight);
        g.drawImage(res.getImage(UserDAO.user.getAvatar()), 0, 0, circleMaskWidth, circleMaskHeight);
        ...
        Label name = new Label(UserDAO.user.getPrenom() + " " + UserDAO.user.getNom());
        ...
        f.getToolbar().addMaterialCommandToSideMenu("My Profil", FontImage.MATERIAL_ACCOUNT_BOX,
                e -> {
                  userDAO.getUserIdByMail(UserDAO.user.getEmail());
                    Demo d = new ProfileForm();
                    d.init(res);
                    Form accform = new Form("My Account", new BorderLayout());
                    accform.add(BorderLayout.CENTER, d.createDemo(accform));
                    Form previous = Display.getInstance().getCurrent();
                    accform.getToolbar().setBackCommand(" ", ee -> {
                        if (d.onBack()) {
                            previous.showBack();
                        }
                    });
        ...
        f.getToolbar().addMaterialCommandToSideMenu("LogOut", FontImage.MATERIAL_WEB, e -> {
            Demo d = new LoginForm();
            d.init(res);
    ...
    m.start();
        ...
        f.getToolbar().addMaterialCommandToSideMenu("About",
                FontImage.MATERIAL_INFO, e -> {
                    Dialog.show("About", "Coolvoiturage "
                            + "Coolvoiurage fo the hole comunity!", "OK", null);
                });
